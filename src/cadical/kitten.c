...
static void log_basic (kitten *kitten, const char *fmt, ...) {
  assert (logging);
  Rprintf ("c KITTEN %u ", kitten->level);
  va_list ap;
  va_start (ap, fmt);
  vprintf (fmt, ap);
  va_end (ap);
...
static void log_reference (kitten *kitten, unsigned ref, const char *fmt,
  ...
  Rprintf (" size %u clause[%u]", c->size, ref);
  ...
  for (all_literals_in_klause (lit, c)) {
    ...
    if (value)
      Rprintf ("@%u=%d", vars[lit / 2].level, (int) value);
  }
  Rprintf ("%c", '\n');
  ((void) 0);
}
...
static void invalid_api_usage (const char *fun, const char *fmt, ...) {
  REprintf ("kitten: fatal error: invalid API usage in '%s': ", fun);
  va_list ap;
  va_start (ap, fmt);
  REvprintf (fmt, ap);
  va_end (ap);
