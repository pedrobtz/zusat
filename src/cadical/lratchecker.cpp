...
void LratChecker::add_original_clause (int64_t id, bool,
  ...
  if (size_clauses && !restore) {
    ...
    if (d) {
      fatal_message_start ();
      REprintf ("%s", "different clause with id ");
      REprintf ("%" PRId64, id);
      REprintf ("%s", " already present\n");
      fatal_message_end ();
    }
  }
  assert (id);
...
#endif
    fatal_message_start ();
    REprintf ("%s", "failed to check derived clause:\n");
    for (const auto &lit : imported_clause)
      REprintf ("%d ", lit);
    REprintf ("%c", '0');
    fatal_message_end ();
...
void LratChecker::add_assumption_clause (int64_t id, const vector<int> &c,
  ...
  for (auto &lit : c) {
    ...
    if (std::find (constraint.begin (), constraint.end (), -lit) !=
      ...
      continue;
    fatal_message_start ();
    REprintf ("%s", "clause contains non assumptions or constraint literals\n");
    fatal_message_end ();
  }
...
void LratChecker::conclude_unsat (ConclusionType conclusion,
  ...
  if (conclusion == CONFLICT) {
    ...
    if (!d || d->size) {
      fatal_message_start ();
      REprintf ("%s", "empty clause not in proof\n");
      fatal_message_end ();
    }
    return;
    ...
    if (ids.size () != 1 || assumption_clauses.size () != 1) {
      fatal_message_start ();
      REprintf ("%s", "expected exactly one assumption clause\n");
      fatal_message_end ();
    }
    if (ids.back () != assumption_clauses.back ()) {
      fatal_message_start ();
      REprintf ("%s", "conclusion is not an assumption clause\n");
      fatal_message_end ();
    }
    return;
  } else {
    ...
    if (constraint.size () != ids.size ()) {
      fatal_message_start ();
      REprintf ("%s", "not complete conclusion given for constraint\n");
      REprintf ("%s", "The constraint contains the literals: ");
    ...
    for (auto &id : ids) {
      if (std::find (assumption_clauses.begin (), assumption_clauses.end (),
        ...
        continue;
      fatal_message_start ();
      REprintf ("%s", "assumption clause for constraint missing\n");
      fatal_message_end ();
    }
  }
}
  ...
  if (d) {
    ...
    for (unsigned i = 0; i < d->size; i++) {
      ...
      if (!mark (lit)) {        // should never happen since ids
        ...
        REprintf ("%s", "deleted clause not in proof:\n");
        for (const auto &lit : imported_clause)
          REprintf ("%d ", lit);
        REprintf ("%c", '0');
        fatal_message_end ();
...
void LratChecker::weaken_minus (int64_t id, const vector<int> &c) {
  ...
  if (d) {
    ...
    fatal_message_start ();
    REprintf ("%s", "weakened clause not in proof:\n");
    for (const auto &lit : imported_clause)
      REprintf ("%d ", lit);
    REprintf ("%c", '0');
    fatal_message_end ();
...
void LratChecker::restore_clause (int64_t id, const vector<int> &c) {
  ...
  if (clauses_to_reconstruct.find (id) == end (clauses_to_reconstruct)) {
    fatal_message_start ();
    REprintf ("%s", "restoring clauses not deleted previously:\n");
    for (const auto &lit : c)
      REprintf ("%d ", lit);
    REprintf ("%c", '0');
    fatal_message_end ();
  ...
  if (!eq) {
    fatal_message_start ();
    REprintf ("%s", "restoring clause that is different than the one imported:\n");
    for (const auto &lit : c)
      REprintf ("%d ", lit);
    REprintf ("%c", '0');
    REprintf ("%s", "vs:\n");
...
void LratChecker::report_status (int, int64_t) {
  ...
  if (num_finalized == num_clauses) {
    ...
    fatal_message_start ();
    REprintf ("%s", "finalize check failed ");
    REprintf ("%" PRIu64, num_clauses);
    REprintf ("%s", " are not finalized");
    fatal_message_end ();
  }
  STOP (checking);
}
...
void LratChecker::dump () {
  ...
  Rprintf ("p cnf %d %" PRIu64 "\n", max_var, num_clauses);
  for (uint64_t i = 0; i < size_clauses; i++)
    for (LratCheckerClause *c = clauses[i]; c; c = c->next) {
      for (unsigned i = 0; i < c->size; i++)
        Rprintf ("%d ", c->literals[i]);
      Rprintf ("0\n");
    }
}
