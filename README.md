...
"At most three of these", "exactly one of these" — the constraints most real
models are built from. CNF cannot express them directly, so they have to be
encoded as clauses:
...
The encoding is chosen automatically, and the choice is not marginal — at 40
literals with `k = 3`, forbidding every 4-subset is 91,390 clauses while a
sequential counter is a few hundred. Override it when you are measuring:

``` r
sat_at_most(s, 1:40, 3, encoding = "sequential")
```

These take a solver rather than returning clauses, because every encoding but
the pairwise one introduces auxiliary variables that must not collide with
